...
template<std::floating_point T>
struct AnimResult {
    std::variant<T, math::Matrix44<T>> variant;

    template<typename SetVal>
    void set(SetVal&& value) {
        static_assert(sizeof(SetVal) == sizeof(math::Matrix44<T>)
                || sizeof(SetVal) == sizeof(T), "Not a valid type to set");
        variant = std::move(value);
    }
    ...
    math::Matrix44<T>* get_mat() {
        return std::get_if<math::Matrix44<T>>(&variant);
    }
};
...
public:
    static TimeTransform<T> translation(
        float               duration,
        float               delay,
    ...
    )
    {
        return TimeTransform<T>{
            ...
            ._res{ math::Matrix44<T>::translation(start_val) },
    ...
    static TimeTransform<T> scaling(
        float               duration,
        float               delay,
        math::Vec3<T>&&     start_val,
        math::Vec3<T>&&     end_val,
    ...
    )
    {
        return TimeTransform<T>{
            ._bezier_curve{ Bezier_curve<T>{ predefined_bezier_values[bezier_type], bezier_type }},
            ._res{ math::Matrix44<T>::scaling(start_val) },
            ._start_val{ start_val },
            ._end_val{ end_val },
            ._duration{ Duration_sec{duration} },
            ._delay{ Duration_sec{delay} },
    ...
    static TimeTransform<T> rotation3d(
        float               duration,
        float               delay,
        math::Vec3<T>&&     start_val,
        math::Vec3<T>&&     end_val,
    ...
    )
    {
        return TimeTransform<T>{
            ._bezier_curve{ Bezier_curve<T>{ predefined_bezier_values[bezier_type], bezier_type }},
            ._res{ math::Matrix44<T>::rotation3d(start_val) },
            ._start_val{ start_val },
            ._end_val{ end_val },
            ._duration{ Duration_sec{duration} },
            ._delay{ Duration_sec{delay} },
    ...
    static TimeTransform<T> rotation_single_axis(
        float               duration,
        float               delay,
        T                   start_val,
        T                   end_val,
    ...
    )
    {
        return TimeTransform<T>{
            ._bezier_curve{ Bezier_curve<T>{ predefined_bezier_values[bezier_type], bezier_type }},
            ._res{ math::Matrix44<T>::rotation(start_val, axis) },
            ._start_val{ start_val },
            ._end_val{ end_val },
            ._duration{ Duration_sec{duration} },
            ._delay{ Duration_sec{delay} },
    ...
    static TimeTransform<T> scalar_val(
        float               duration,
        float               delay,
        T                   start_val,
        T                   end_val,
    ...
    )
    {
        return TimeTransform<T>{
            ...
            ._duration{ Duration_sec{duration} },
            ._delay{ Duration_sec{delay} },
            ._anim_type = math::TransformationType::SCALAR,
            ._loop = loop,
            ._is_delay_passed = delay == 0.0f
        };
    }

    static TimeTransform<T> shear(
        float                   duration,
        float                   delay,
    ...
    )
    {
        return TimeTransform<T>{
            ._bezier_curve{ Bezier_curve<T>{ predefined_bezier_values[bezier_type], bezier_type }},
            ._res{ math::Matrix44<T>::shearing(start_val, axis) },
            ._start_val{ start_val },
            ._end_val{ end_val },
            ._duration{ Duration_sec{duration} },
            ._delay{ Duration_sec{delay} },
    ...
    AnimResult<T>& update(Duration_sec delta_time) {
        if (_is_paused || (_loop == Loop_type::NONE && _is_ended)) {
            return _res;
        }
        if (!_is_started) {
        ...
        switch (this->_anim_type) {
            case math::TransformationType::TRANSLATION: {
                ...
                math::Matrix44<T>* res_unwrapped = _res.get_mat();
                assert(res_unwrapped && "Res is not proper type");
                res_unwrapped->translate(lerp_val);
                return _res;
            } break;
            ...
            case math::TransformationType::ROTATION3d: {
                ...
                math::Matrix44<T>* res_unwrapped = _res.get_mat();
                assert(res_unwrapped && "Res is not proper type");
                res_unwrapped->rotate3d(lerp_val);
                return _res;
            } break;
            case math::TransformationType::ROTATION: {
                ...
                math::Matrix44<T>* res_unwrapped = _res.get_mat();
                assert(res_unwrapped && "Res is not proper type");
                res_unwrapped->rotate(lerp_val, _axis);
                return _res;
            } break;
            case math::TransformationType::SHEAR: {
                ...
                math::Matrix44<T>* res_unwrapped = _res.get_mat();
                assert(res_unwrapped && "Res is not proper type");
                res_unwrapped->shear(_axis, lerp_val);
                return _res;
            } break;
            case math::TransformationType::SCALAR: {
                const T* start_unwrapped = _start_val.get_scalar();
                const T* end_unwrapped = _end_val.get_scalar();
                assert(start_unwrapped && end_unwrapped && "Scalar expected from unwrapping");
                _res.set(math::Global::lerp(*start_unwrapped, *end_unwrapped, linear_0to1));
                return _res;
            } break;
            default:
...
public:
    math::Vec3<T>           _velocity;
    math::Vec3<T>           _acceleration;
};
}
